> 관련 문서: [[11. KickOFF]] · [[22. Cost Plan]] · [[24. 기능명세서 v1]]
...
> **2026-09-03 아키텍처 전환**: 공식 커리큘럼 가이드라인 확인 결과 2차 프로젝트는 "Azure Databricks 프로젝트"(Data Factory+Databricks 실시간 데이터 처리)로 명명돼 있음 — ML 기술 핵심을 로컬 학습에서 **Databricks**로 전환. 4·6·롤별 표 갱신, Dataverse/Teams 흐름은 차별화 액션 레이어로 유지.
...
## 0. 제품 정체성 (재확인)
- **만드는 것**: **Fashion AI Agent Appservice**(가칭) — **Databricks+Data Factory로 처리하는 AI 재고 리플레니시먼트(수요계획+발주승인) 모델**을, SAP 등 기존 ERP는 건드리지 않고 Dataverse(Power Platform) 기반 Teams 승인 액션으로 연결하는 PoC
...
- 정책문서 질의응답 형태의 실제 RAG는 Tier 2 백로그로 유보
...
## 5. 다음 논의 (Week 0)
- [ ] Teams 승인 카드에 "예측근거 자연어 설명" GenAI 기능을 붙일지 (Tier 1 스트레치 후보) — 기술 설계 초안은 7절 참고(2026-08-26 추가) #todo/week0
...
> **2026-08-29 프레이밍 메모**: 아래 "탐색" 단계는 [[OBSIDIAN/2026_MS_DATASCHOOL/20.PROJECT/22.MS_2nd_Project/00.MS_2nd_Project_PM/30.DATA/31. NQNQ_data Frame/70. Data Model & Architecture/71. Entity Definitions & Data Dictionary]] ERD의 엔티티 관계(SKU→판매속도→리드타임→프로모션)를 순회하는 구조라, 사실상 **naive 벡터 RAG가 아니라 그래프 기반 근거 탐색(GraphRAG류)**임 — 새 그래프 DB를 추가할 필요 없이 기존 ERD로 이미 확보된 특성이라 이 이름으로 재서술만 함. 온톨로지 신규 구축·전용 Graph DB(Neo4j 등)·완전 자율 Agent Harness는 검토 후 채택 보류 — 이미 정형 데이터(ERD)가 있어 온톨로지가 불필요하고, Agent Harness는 아래 "왜 완전 자동생성이 아닌가"의 Human-in-the-loop 설계와 정면 충돌하기 때문. 장기 아이디어로는 8절 백로그에 유효.

### 파이프라인 (ReAct 축소판)
1. **탐색(Reasoning)**: risk_score 산출에 쓰인 근거 데이터를 조회 — 최근 N일 판매속도, 리드타임 대비 남은 재고소진일, 카테고리별 계절 민감도 계수([[OBSIDIAN/2026_MS_DATASCHOOL/20.PROJECT/22.MS_2nd_Project/00.MS_2nd_Project_PM/30.DATA/31. NQNQ_data Frame/70. Data Model & Architecture/74. Real-World Data Grounding (Weather & Body)]]), 임박한 프로모션 유무
...
### 의존성
- Azure OpenAI 학교 커리큘럼 학습 완료됨 — 이미 Tier 1 스트레치로 분류돼 있음
- risk_score 산출 공식이 먼저 확정돼야 "근거 데이터" 항목이 정해짐 → 공식은 [[OBSIDIAN/2026_MS_DATASCHOOL/20.PROJECT/22.MS_2nd_Project/01.MS_2nd_Project_Team/20.ARCHITECTURE/23. Alert & Trigger Rules]]에서 신규 확정(2026-08-26)
...
**1차 프로젝트 대비 의미**: 1차 RAG는 "질문→리포트 생성, 끝"이었음. 우리 코어는 이미 "예측→승인→발주 액션"까지 가서 그 자체로 앞서 있는데, 여기에 MAI식 대화형 레이어까지 얹으면 "액션으로 이어지는 대화형 의사결정 코파일럿"이라는, 1차와 완전히 다른 카테고리의 포지셔닝이 가능함. 발표에서 MATCH/MAI를 실제 벤치마킹 사례로 인용 가능.
...
> 아래는 피벗 이전 초안을 그대로 보존한 것. NQNQ를 지그재그 내부 실제 시스템처럼 서술하고 있지만, 지금 시점에서는 "우리 제품을 도입한 회사가 언젠가 이 수준까지 확장한다면"이라는 장기 가정으로 읽어야 함. 장기 비전(Tier 3)에 해당.
